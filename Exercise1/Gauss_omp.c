...
int main(int argc, char **argv) {
	...
	for (k = 0; k < N-1 ; k++) {
		#pragma omp parallel for shared(k,N,A) private(i,j,l)
		for (i = k+1; i < N ; i++) {
			l = A[i][k]/A[k][k];
			//printf("l == %lf\n",l);
			for (j = k+1;j<N;j++) {
	...
	printf("Gauss OpenMP: Elapsed calculation time: %lf seconds\n", mtime);
	...
	printf("Gauss OpenMP: Elapsed total time: %lf seconds\n", total);
